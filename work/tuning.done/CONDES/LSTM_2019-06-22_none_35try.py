...
def import_dataset():
    DS = pd.read_csv(ROOT+"data/dump-Las_Condes_2018-04-12_230000-verano.csv")
...
def precalcular_agregados(REMAKE=False):
    try:
        if REMAKE == False:
            agdf = pd.read_csv(ROOT+"precalcs/precalc_agregados.csv")
        ...
        agdf.to_csv(ROOT+"precalc_agregados.csv")
...
def precalcular_eventos(theta,REMAKE=False):
    print("Precalculandos Eventos Criticos ...")
    try:
        if REMAKE == False:
            ecdf = pd.read_csv(ROOT+"precalcs/precalc_ECat%.2f.csv"%theta, index_col="registered_on", parse_dates=True)
            ecdf = ecdf.asfreq("D")
        else:
            pd.read_csv("nunca_nadie_me_encontrara.nunca_jamas")
    except:
        ...
        ecdf.to_csv(ROOT+"precalcs/precalc_ECat%.2f.csv"%theta, index=True)

    return ecdf

asdf = precalcular_eventos(61,REMAKE=False)
...
def import_merge_and_scale(Config, verbose=True, SCALE = True):
    ...
    ecdf = precalcular_eventos(THETA, REMAKE=False)
    ...
    if FILTER_YEARS:
        i,f = FILTER_YEARS
        norm_data = norm_data["%d-11-01"%i:"%d-03-31"%f]
    dataset = norm_data
...
def nonancheck():
    tempConfig = {
        ...
        "IMPUTATION":None,
        "FILTER_YEARS" : [],
        "AGREGADOS":[], "PRECALC":precalcular_agregados(),
        "THETA":61,
        "TARGET":"O3",
        "SHIFT":-1,
        "PAST":False,
...
def plot_y_true():
    ...
    df.asfreq("D").iplot(title="TARGET: %s, index: %d"%(TARGET,f))
...
def make_folds_TVT(internalConfig, Config):
    ...
    for year in range(firstyear, 2018-2):
        ...
        if len(trainX) != 0 and len(validX) != 0 and len(testX) != 0:
            ...
            print("{} excluido, lags: {}, examples:".format(year,LAGS), (len(trainX),len(validX),len(testX)) )
...
def create_filename(internalConfig, Config):
    ...
    strTD = "TD"*TIMEDIST
    FILE_NAME = "./%s/%s-%.2f-(%d,%d,%d)%s-%s-%.3f-%s-%s-%d-(%s,%s)-%s-%s%sSHU%s%s%s%s%s.h5"%(MODELS_FOLDER, MODEL_NAME, THETA, BATCH_SIZE, TIMESTEP, f_len, strQUANTILES, strFEATURES, TRAINPCT, TARGET, LAYERS, EPOCHS, str(PAST), FUTURE, strTD, DROP_RATE, isLSTM , SHUFFLE,TVT, fyears,IMPUT, qfileHash )
...
# ## RMSEatθ
# Calcula el RMSE sólo cuando el valor del 'y' real es mayor o igual a θ (theta).
...
def myLSTMPredict(internalConfig, Config):
    ...
    for fold in range(0,len(list_trainX)):
        ...
        # calculate root mean squared error
        trainScore = math.sqrt(mean_squared_error(trainYinv, trainPred))
        trainScoreAll.append(trainScore)
        validScore = math.sqrt(mean_squared_error(validYinv, validPred))
        validScoreAll.append(validScore)
        testScore = math.sqrt(mean_squared_error(testYinv, testPred))
        testScoreAll.append(testScore)
        
        
        trainScore = RMSEat(THETA,trainYinv, trainPred)
        trainScoreAtTHETA.append(trainScore)
        validScore = RMSEat(THETA,validYinv, validPred)
        validScoreAtTHETA.append(validScore)
        testScore = RMSEat(THETA,testYinv, testPred)
        testScoreAtTHETA.append(testScore)
        
        tempTHETA = THETA
        THETA=61
        trainScore = RMSEat(THETA,trainYinv, trainPred)
        trainScoreAt61.append(trainScore)
        validScore = RMSEat(THETA,validYinv, validPred)
        validScoreAt61.append(validScore)
        testScore = RMSEat(THETA,testYinv, testPred)
        testScoreAt61.append(validScore)
        THETA= tempTHETA
    ...
    print("Error RMSE")
    print('    Train Score     : %.2f RMSE' % (np.nanmean(trainScoreAll)))
    print('    Validation Score: %.2f RMSE' % (np.nanmean(validScoreAll)))
    print('    Test Score      : %.2f RMSE' % (np.nanmean(testScoreAll)))
    
    
    print("RMSEat%d"%THETA)
    print('    Train Score     : %.2f RMSEat%d' % (np.nanmean(trainScoreAtTHETA),THETA))
    print('    Validation Score: %.2f RMSEat%d' % (np.nanmean(validScoreAtTHETA),THETA))
    print('    Test Score      : %.2f RMSEat%d' % (np.nanmean( testScoreAtTHETA),THETA))
    
    tempTHETA = THETA
    THETA=61
    print("RMSEat%d"%THETA)
    print('    Train Score     : %.2f RMSEat%d' % (np.nanmean(trainScoreAt61),THETA))
    print('    Validation Score: %.2f RMSEat%d' % (np.nanmean(validScoreAt61),THETA))
    print('    Test Score      : %.2f RMSEat%d' % (np.nanmean( testScoreAt61), THETA))
    THETA=tempTHETA
    ...
    validScore = math.sqrt(mean_squared_error(validYinv, validPred))
    print(' Validation Score: %.2f RMSE' % (validScore))
    testScore = math.sqrt(mean_squared_error(testYinv, testPred))
    ...
    validScore = RMSEat(THETA,validYinv, validPred)
    print('    Train Score: %.2f RMSEat%d' % (validScore,THETA))
    testScore = RMSEat(THETA,testYinv, testPred)
    ...
    return trainPred, validPred, testPred
...
#myLSTMPredict(myLSTMoutput, LSTMconfig)
...
def Qprediction(internalConfig, Config):
    ...
    for fold in range(0,len(list_trainX)):
        ...
        # calculate root mean squared error
        trainScore = math.sqrt(mean_squared_error(finalTrainY, finalTrainPredq))
        trainScoreAll.append(trainScore)
        validScore = math.sqrt(mean_squared_error(finalValidY, finalValidPredq))
        validScoreAll.append(validScore)
        testScore = math.sqrt(mean_squared_error(finalTestY, finalTestPredq))
        testScoreAll.append(testScore)
        
        
        trainScore = RMSEat(THETA,finalTrainY, finalTrainPredq)
        trainScoreAtTHETA.append(trainScore)
        validScore = RMSEat(THETA, finalValidY, finalValidPredq)
        validScoreAtTHETA.append(validScore)
        testScore = RMSEat(THETA, finalTestY, finalTestPredq)
        testScoreAtTHETA.append(testScore)
        ...
        validScoreAt61.append(validScore)
        testScore = RMSEat(THETA, finalTestY, finalTestPredq)
        testScoreAt61.append(testScore)
        THETA= tempTHETA
    ...
    '''
    #else:
    #    finalTrainPredq = "DUMMY"
    #    finalTrainY     = "DUMMY"
    #    finalValidPredq = "DUMMY"
    #    finalValidY     = "DUMMY"
    #    finalTestPredq  = "DUMMY"
    #    finalTestY      = "DUMMY"
